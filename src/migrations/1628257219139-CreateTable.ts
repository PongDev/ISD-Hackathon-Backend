...
export class CreateTable1628257219139 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`ProjectDatabase\`.\`user\` (\`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`displayName\` varchar(255) NOT NULL, PRIMARY KEY (\`email\`)) ENGINE=InnoDB`,
    );
  }
